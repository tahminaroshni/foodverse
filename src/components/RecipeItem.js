...
const RecipeItem = ({ handleFavouriteItem, savedFavouriteItem }) => {
  const { id } = useParams();
  const { data: recipe, loading, error } = useFetch(id);
  const [isSaved, setIsSaved] = useState(null);
  const navigate = useNavigate();

  const { publisher, title, servings, cooking_time, image_url, ingredients, source_url } = recipe;
  ...
  // check recipe item saved or not
  useEffect(() => {
    if (recipe) {
          ...
          <div className="recipe-item-section grid grid-cols-2 gap-10 py-12 container mx-auto justify-center">
            <div className="recipe-item-left">
              <div className="  overflow-hidden">
                <img
                  className="w-full object-cover rounded-lg hover:scale-110 duration-300"
                  src={image_url}
                  alt={title} />
              </div>
            ...
            <div className="recipe-item-right flex flex-col gap-5 items-start">
              ...
              <div className="buttons flex gap-5 uppercase">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-3 px-5 rounded-md bg-sky-400 text-sky-50  hover:bg-gray-600 duration-300 text-sm">Get Directions
                </a>

                <NavLink
                  to='/favourites'
                  className={`flex gap-2 text-sm p-3 px-5 rounded-md bg-sky-200 text-sky-500 hover:bg-sky-500 hover:text-sky-50 font-medium uppercase tracking-wider duration-300 ${isSaved && ' bg-orange-500 text-orange-50 hover:bg-orange-200 hover:text-orange-500 duration-300'}`}
